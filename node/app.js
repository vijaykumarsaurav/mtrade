...
app.post('/saveCurrentSassion', function (req, res) {
  
  con.query('delete from sessioninfo', function (err, result) {
    if (err) throw err;
    console.log(" user token not deleted", new Date());

    //create table sessioninfo(feedToken varchar(500), jwtToken varchar(500), refreshToken varchar(500) );
    var data = req.body;
    let feedToken = data.feedToken; 
    let jwtToken = data.jwtToken; 
    let refreshToken = data.refreshToken;
      var sql = `insert into sessioninfo (feedToken,jwtToken,refreshToken) VALUES ('${feedToken}','${jwtToken}','${refreshToken}')`;
 
      
    //   var values = [{feedToken, jwtToken, refreshToken}];

     console.log(" sql",  sql);

      con.query(sql, function (err, result) {
        if (err) throw err;
        console.log(" user token added", new Date());
        res.status(200).send({ status: 200, result: 'user token added' });
        return;
      });
...
app.get('/sectorAction', function (req, res) {
  console.log("sector",  req.body);

  let sectorInfo = req.body;
  sectorInfo.time = new Date().toLocaleDateString(); 

  fs.writeFile('sectorUpdate.json', JSON.stringify(sectorInfo), 'utf8', function callback() {
    console.log("Added", sectorInfo);
    res.status(200).send({ data: sectorInfo, status :"added"});

    var sql = "select *  FROM userTokens";
    con.query(sql, function  (err, result) {
      if (err) throw err;
      console.log("user tokens",result.length);
      req.userTokens = result; 
