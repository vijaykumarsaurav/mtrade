...
class VerifyEdit extends React.Component {
                ...
                if(selectedProduct){
                    this.setState({
                        ...
                        if(this.state.showPersonalDetails){
                            this.setState({ rejectedReasons: this.state.bothReasons.qvaRejectionReasons});
                        }
    ...
    render() {
        ...
        return(
            ...
            <React.Fragment>
                <PostLoginNavBar/>           
                <Typography variant="h6" style={styles.textStyleHeading}>QVA View and Verify Document</Typography>
    ...
    onlockTransectionOnSkip = (txn) =>{
        ...
        ActivationService.unlockQVATransectionsSkip( transactionsIds ).then(res => {
            let data = resolveResponse(res);
            if(data.message != 'ok'){
