...
import GroupByButton from './tools/GroupByButton';
...
class OrderBook extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            pattenNamePending: "",
            searchSymbolPending: "",
            autoSearchList: [{ "token": "26009", "symbol": "BANKNIFTY", "name": "BANKNIFTY", "expiry": "", "strike": "-1.000000", "lotsize": "-1", "instrumenttype": "", "exch_seg": "NSE", "tick_size": "-1.000000" },  {
                "token": "2",
                "symbol": "NIFTY50",
                "name": "NIFTY",
                "expiry": "",
            ...
            switchOrderType: false,
            orderStrikePrice : 0, 
            optionStrikePriceValue: 0, 
            optionLotSize : 0, 
            OrderQtyLot: 1,
            availableFund: 0,
            buyAboveBelow  : "Buy Above", 
            sellAboveBelow  : "Sell Below", 
        }
    }
    ...
    addInOrderPenidngList = async () => {
        ...
        if (this.state.searchSymbolPending && this.state.buyAtPending || this.state.sellAtPending) {
            ...
            var data = {
                ...
                buyAt: this.state.switchOrderType === false ? this.state.buyAtPending : "", 
                sellAt: this.state.switchOrderType === true ? this.state.buyAtPending : "",
                ...
                orderStrikePrice : this.state.orderStrikePrice, 
                optionStrikePriceValue: this.state.optionStrikePriceValue, 
                optionLotSize : this.state.optionLotSize, 
                OrderQtyLot: this.state.OrderQtyLot,
                buyAboveBelow  : this.state.buyAboveBelow, 
                sellAboveBelow  : this.state.sellAboveBelow, 
            }
    ...
    placeOptionSPLevelOver = (indexData, spotPrice) => {
        ...
        if (indexData.buyAt) {
            ...
            this.props.buyOption("CE", indexData.symbol, indexData.optionStrikePriceValue, nextExp, indexData.OrderQtyLot, indexData);
            ...
            this.props.buyOption("PE", indexData.symbol, indexData.optionStrikePriceValue, nextExp, indexData.OrderQtyLot, indexData);
        }
        ...
        for (let index = 0; index < this.state.orderPenidngOptionList.length; index++) {
            ...
            AdminService.getLTP(data).then(res => {
                ...
                if (LtpData && LtpData.ltp) {
                    ...
                    if (element.buyAt && (element.buyAboveBelow == 'Buy Above' && LtpData.ltp >= parseFloat(element.buyAt)) || (element.buyAboveBelow == 'Buy Below' && LtpData.ltp <= parseFloat(element.buyAt))) {
                    ...
                    } else if (element.sellAt && (element.sellAboveBelow == 'Sell Below' &&  LtpData.ltp <= parseFloat(element.sellAt)) || (element.sellAboveBelow == 'Sell Above' &&  LtpData.ltp >= parseFloat(element.sellAt))) {
                        var isDelete = this.deleteInOrderPenidngList(element);
                        if (isDelete) { // && !this.state.buyOptionFlag
                            this.setState({ buyOptionFlag: true }, function () {
                                this.placeOptionSPLevelOver(element, LtpData.ltp);
    ...
    getTradePrice = () => {
     ...
     //   console.log('lastTradedData', lastTradedData)
        ...
        AdminService.getFunds().then(fundsRes => {
            // console.log('profiledata', profileRes); 
             ...
             //  var fundsResData =  fundsRes && fundsRes.data; 
               if(fundsResData.status && fundsResData.data){
                this.setState({  availableFund : Math.round(fundsResData.data.net) }); 
               }
        })
    }
    ...
    getStrikePrice =()=> {
        let optionStrikePriceValue = 0, optionLotSize = 0; 
        if(this.state.lastTradedData && this.state.lastTradedData.ltp){
            if(this.state.lastTradedData.symbol === 'NIFTY'){
                optionLotSize = 50; 
                optionStrikePriceValue = (Math.round(this.state.lastTradedData.ltp) - Math.round(this.state.lastTradedData.ltp) % 100) + (this.state.orderStrikePrice * optionLotSize)
            }else if(this.state.lastTradedData.symbol === 'BANKNIFTY'){
                optionLotSize = 100; 
                optionStrikePriceValue = (Math.round(this.state.lastTradedData.ltp) - Math.round(this.state.lastTradedData.ltp) % 100) + (this.state.orderStrikePrice * optionLotSize)
            }
        }
        this.setState({optionStrikePriceValue: optionStrikePriceValue, optionLotSize: optionLotSize}); 
    }

    updateStrikePrice = (e) => {
         if(parseInt(e.target.value) > -5 && parseInt(e.target.value) < 5){
            this.setState({[e.target.name]: e.target.value}, () => {
                this.getStrikePrice(); 
            }); 
        }
    ...
    render() {
        ...
        return (
            <React.Fragment>
                ...
                <Paper style={{ overflow: "auto", padding: '5px', background: this.state.switchOrderType ? 'antiquewhite' : 'aliceblue' }} >

                    <Grid justify="space-between"
                        container>
                        <Grid item>
                            <Typography color="primary" gutterBottom>
                                Nifty & Banknifty Option ({this.state.orderPenidngOptionList && this.state.orderPenidngOptionList.length})
                                {window.location.hash != "#/position-new" ? <Button onClick={() => this.backToPositionPage()}> Position page </Button> : ""}
                            </Typography>
                        </Grid>
                        ...
                        <Grid item >
                            <Grid container spacing={2}>
                                ...
                                <FormGroup>
                                    <FormControlLabel
                                    control={<Switch checked={this.state.switchOrderType} name="switchOrderType" onChange={this.switchChange} aria-label="Order Type" />}
                                    label={this.state.switchOrderType ? <Typography color="secondary" gutterBottom> SELL </Typography>  : <Typography color="primary" gutterBottom> BUY </Typography>}
                                    />
                                </FormGroup>
                                </Grid>

                                <Grid item>
                                    <TextField label="Atm0 Itm+ Otm-"  type="number" name="orderStrikePrice" value={this.state.orderStrikePrice} onChange={this.updateStrikePrice} />
                                    <br /> Strike Price: <b>{this.state.optionStrikePriceValue}</b> &nbsp;  Option Price: <b>{this.state.optionStrikePriceValue}</b>  
                                </Grid>

                                <Grid item>
                                    <TextField label="No of Lot"  type="number" name="OrderQtyLot" value={this.state.OrderQtyLot} onChange={this.updateQuanty} />
                                    <br /> Lot Size :{this.state.optionLotSize} &nbsp; Qty: <b>{this.state.optionLotSize * this.state.OrderQtyLot}</b>  &nbsp; Avl Fund: {this.state.availableFund}
                                </Grid>

                                {/* <GroupByButton switchOrderType={this.state.switchOrderType} /> */}
                             
                                {this.state.switchOrderType ? <Grid item>
                                    <FormControl style={styles.selectStyle} >
                                        <InputLabel>Sell Above/Below</InputLabel>
                                        <Select value={this.state.sellAboveBelow} name="sellAboveBelow" onChange={this.updateInput}>
                                            <MenuItem value={'Sell Above'}>{'Sell Above -  Against'} <ArrowUpwardIcon fontSize="small" /></MenuItem>
                                            <MenuItem value={'Sell Below'}>{'Sell Below'} <ArrowDownwardIcon fontSize="small"/></MenuItem>
                                        </Select>
                                    </FormControl>
                                </Grid> : <Grid item>
                                    <FormControl style={styles.selectStyle} >
                                        <InputLabel>Buy Above/Below</InputLabel>
                                        <Select value={this.state.buyAboveBelow} name="buyAboveBelow" onChange={this.updateInput}>
                                            <MenuItem value={'Buy Above'}>{'Buy Above'}<ArrowUpwardIcon fontSize="small"/></MenuItem>
                                            <MenuItem value={'Buy Below'}>{'Buy Below - Against'}<ArrowDownwardIcon fontSize="small"/></MenuItem>
                                        </Select>
                                    </FormControl>
                                </Grid>}
                                ...
                                <Grid item>
                                    <TextField label="Order Price" type="number" name="buyAtPending" value={this.state.buyAtPending} onChange={this.updateInput} />
                                    <br /> High: {this.state.lastTradedData.high} Low: {this.state.lastTradedData.low}
                                </Grid>


                                {/* <Grid item>
                                    <TextField label="Sell Spot Price" type="number" name="sellAtPending" value={this.state.sellAtPending} onChange={this.updateInput} />
                                    <br /> Low: {this.state.lastTradedData.low}
                                </Grid> */}


                                <Grid item>
                                    <TextField label="Pattern Name" name="pattenNamePending" value={this.state.pattenNamePending} onChange={this.updateInput} />
