...
class Home extends React.Component {
    ...
    getOptionStockLiveLtp = async () => {
     ...
     //   console.log('activeStockOptions',this.state.activeStockOptions);
            ...
            AdminService.getLTP(data).then(res => {
                ...
                if (LtpData && LtpData.ltp) {
                    ...
                    for (let indexP = 0; indexP < this.state.positionList.length; indexP++) {
                        let elementP = this.state.positionList[indexP]; 
                        if(elementP.symbolname, element.optionUnderlineName, elementP.symbolname === element.optionUnderlineName){
                            selectedPosition = this.state.positionList[indexP];
                            break; 
                        }
                    }
                    element.optionUnderlineLtp = LtpData.ltp; 
                    element.optionUnderlineName = element.name; 
                    element.optionUnderlineChange = element.perChange; 
                    let optionFlag = selectedPosition.instrumenttype === "OPTIDX" ? true : false; 

                    //ce quireoff 
                    if (selectedPosition.optiontype === 'CE' && (LtpData.ltp < element.optionStockStoploss || LtpData.ltp >= element.optionStockTarget)) {
                        var isDelete = this.deleteActiveOptionStock(element);
                        if (isDelete) { 
                            this.squareOff(selectedPosition, optionFlag);
                        }
                    }
                    //pe squired off
                    if (selectedPosition.optiontype === 'PE' && (LtpData.ltp > element.optionStockStoploss || LtpData.ltp <= element.optionStockTarget)) {
                        var isDelete = this.deleteActiveOptionStock(element);
                        if (isDelete) {     
                            this.squareOff(selectedPosition, optionFlag);
                        }
        ...
        AdminService.getPosition().then(res => {
            ...
            if (positionList && positionList.length > 0) {
                ...
                for (let index = 0; index < positionList.length; index++) {
                    ...
                    if((element.optiontype == 'CE' || element.optiontype == 'PE') && parseInt(element.netqty) > 0){ //&& element.instrumenttype == "OPTSTK" && element.netqty > 0 
                      let found =  this.state.activeStockOptions.filter( name => name.name == element.symbolname);
                      
                      if(!found[0] && CommonMethods.getStockTokenDetails(element.symbolname)){
                            this.setState({ activeStockOptions : [ ...this.state.activeStockOptions, CommonMethods.getStockTokenDetails(element.symbolname)]})  
                       }  
                    }
                    for (let index = 0; index < this.state.activeStockOptions.length; index++) {
                        const activeOption = this.state.activeStockOptions[index];
                        if(activeOption && activeOption.name == element.symbolname){
                            element.optionUnderlineName= activeOption.optionUnderlineName;
                            element.optionUnderlineLtp = activeOption.optionUnderlineLtp;
                            element.optionUnderlineChange = activeOption.optionUnderlineChange;
                            let trackSLPrice = localStorage.getItem('trackSLPrice') ? JSON.parse(localStorage.getItem('trackSLPrice')) : [];
                            let found = trackSLPrice.filter(item => item.tradingsymbol == element.symbolname); 
                            if(!this.state[element.tradingsymbol+'stoploss'] && found.length>0 && found[0].priceStopLoss){
                                element.optionStockStoploss = found[0].priceStopLoss; 
                            }
                            if(!this.state[element.tradingsymbol+'target'] && found.length>0 && found[0].priceTarget){
                                element.optionStockTarget = found[0].priceTarget; 
                            }
                            break;
                        }else{
                            element.optionUnderlineName= '-';
                        }
                    }
    ...
    optionStockStoplossChange = (e) => {
        ...
        for (let index = 0; index < this.state.activeStockOptions.length; index++) {
            ...
            if(e.target.name === element.name){
                element.optionStockStoploss = parseFloat(e.target.value); 
                break;
            }
        }
    }
    optionStockTargetChange = (e) => {
        ...
        for (let index = 0; index < this.state.activeStockOptions.length; index++) {
            const element = this.state.activeStockOptions[index];
            if(e.target.name === element.name){
                element.optionStockTarget = parseFloat(e.target.value); 
                break;
            }
        }
    }
                ...
                <Grid style={{ padding: '5px' }} justify="space-between" direction="row" container>
                    <Grid item >
                        ...
                        <Typography color="primary" gutterBottom>
                            Positions ({this.state.positionList && this.state.positionList.length})    <span id="niftySpid">  </span>  &nbsp;&nbsp;  <span id="bankniftySpid"> </span>
                        </Typography>
                    ...
                    <Grid item>
                        <Typography component="h3"  {...window.document.title = "P/L " + (this.state.todayProfitPnL - this.state.totalExpence).toFixed(2)}>
                                    ...
                                    {this.state.positionList ? this.state.positionList.map(row => (
                                        row.producttype !== 'DELIVERY1' ? <TableRow hover key={row.symboltoken} style={{ background: row.netqty !== '0' ? 'lightgray' : "" }} >
                                            ...
                                            <TableCell width={'20%'} align="left">
                                                <p style={{ color: row.optionUnderlineChange > 0 ? "green" : "red" }} size="small" variant="contained" title="Candle refresh" onClick={() => this.refreshCandleChartManually(row)} >
                                                &nbsp;  {row.optionUnderlineName ? `${row.optionUnderlineName} ${row.optionUnderlineLtp ? row.optionUnderlineLtp : ""} (${row.optionUnderlineChange ? row.optionUnderlineChange : ""}%)`: '-'}
                                                </p>
                                            </TableCell>

                                            <TableCell align="left">
                                                <input disabled={!row.optionUnderlineName} step="0.5" style={{ width: '40%', textAlign: 'center' }} type='number' value={row.optionStockStoploss} name={row.tradingsymbol+'stoploss'} onChange={this.optionStockStoplossChange} /> 
                                            </TableCell>
                                            <TableCell align="left">
                                                 <input disabled={!row.optionUnderlineName} step="0.5" style={{ width: '40%', textAlign: 'center' }} type='number' value={row.optionStockTarget} name={row.tradingsymbol+'target'} onChange={this.optionStockTargetChange} />
                                            </TableCell>
                                            ...
                                            <TableCell style={{ cursor: 'help', color:'red' }} title={row.expenceInfo} align="left">{row.tradeExpence}</TableCell>
