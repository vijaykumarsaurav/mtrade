...
const BNOptionBuyAtLevel = ({
  ...
  const placeOptionSPLevelOver= (spotPrice, optionType, spottype, elementInfo)=>{
      ...
      let expiry = moment(nextExpiryOption.expiry).format('DDMMMYY');


      let tradingsymbol = `BANKNIFTY${expiry.toUpperCase()}C${strikePrice}`;  //BANKNIFTY15SEP22C40400 "BANKNIFTY15SEP22C41300"
      if(nextExpiryOption){
        let optionInput = {
          'buy_or_sell': 'B',
          'product_type': 'M',
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        Shoonya Order <button onClick={() => resetInput()}>Reset all</button> Nifty Bank Option buy on Level {LiveLtp && LiveLtp.iv}
        </Typography>
        <br />
