...
class Home extends React.Component{
    ...
    makeConnection = (wsClint) => {
       ...
       let subReq =   {
        ...
        if(this.wsClint.readyState === this.wsClint.OPEN){
            wsClint.send(JSON.stringify(subReq));
    ...
    componentDidMount() {
        ...
        this.setState({ feedToken: feedToken, clientcode: clientcode, jwtToken :  tokens.jwtToken}, function () {
            let url = `wss://smartapisocket.angelbroking.com/websocket?jwttoken=${tokens.jwtToken}&&clientcode=${clientcode}&&apikey=I4O6PJAn`;

            this.wsClint = new w3cwebsocket(url);

            console.log("wsClint", this.wsClint)
